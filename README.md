...
<!-- README-NAV-START -->
> **BIG PAGE — do NOT read this file whole.** It is 215,076 bytes, roughly 54k tokens. One whole-file Read truncates at 25,000 tokens and returns only the first ~46% of it, so answering from that view means answering from a fraction of the page. Read one section instead:
>
...
- **My Deck** opens the combined card gallery and deck editor from the title screen.
...
- `materials/Convergence card stat excel sheet.xlsx` is the project's only card workbook. It is generated from both live CSV rosters, the engine's rarity names, and this page's Rules at a glance. Keep the Cards, Relics, Rules and Summary tabs; do not maintain a second spreadsheet or edit this one as a separate card authority. `npm run sync:cards` refreshes it, preserves typed stats and recalculated summaries, and updates its adjacent `.sync.json` record. The record binds the source content and generator to the actual workbook bytes, so edits, missing files and stale copies are detected.
- Workbook refresh runs before development, validation and production builds, and in the normal `npm run check` workbook suite. The development server also watches the roster, rarity vocabulary, generator and rule-summary sources. Pages deployment independently runs `node source/scripts/sync-card-workbook.mjs --check` and refuses to publish a missing, altered or stale workbook. Refreshing requires the bundled Codex spreadsheet runtime; the release freshness check requires only Node. A locked workbook or missing runtime blocks the operation with a corrective error instead of accepting stale output. `CONVERGENCE_ARTIFACT_RUNTIME` may point to the bundled dependency directory on another machine.
- `node scripts/check-card-workbook.mjs --exercise` verifies automatic regeneration with disposable source changes: zero stats, quoted multiline text, added and removed cards/relics, tampering and invalid source. It reads the exported XLSX independently with bundled Python, checks every live card value, cached totals, rules, table filters and frozen panes, and removes its temporary workbooks. The unchanged-data path preserves the existing workbook rather than rewriting it on every command.
...
Voice auditions: full boss-audio integration is paused for the owner to compare engines. `materials/voice-auditions/` contains four Qwen samples and five Kokoro samples, plus labelled playlists. The first four use matching text. `npm run voice:audition` is the bounded comparison front door; the Qwen renderer defaults to only those four keys. Qwen3-TTS 1.7B VoiceDesign runs in the isolated `.preview/voice-runtime` with PyTorch 2.7.1 CUDA 12.8, using the measured four-line batch and sequential waveform decoding (4.56 GB allocated at the benchmark). The already-finished full Qwen batch remains private under `.preview/voice-full-hold`; it is not included in the game. Story dialogue remains text-only until the owner selects an engine.
